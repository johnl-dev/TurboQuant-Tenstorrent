...
def load_fp16_dump(path: str, n: int, d: int, tile: int = 32) -> np.ndarray:
    ...
    return fp16[:n * d].reshape(n, d).view(np.float16).astype(np.float64)
...
def load_qjl_dump(path: str, n: int, k: int, tile: int = 32
    ...
    for v in range(n):
        ...
        # r_norm (FP16 → float64)
        fp16_val = np.frombuffer(raw[base + qjl_bytes:base + qjl_bytes + 2],
                                  dtype=np.uint16)[0]
        r_norm_out[v] = float(np.array(fp16_val, dtype=np.uint16).view(np.float16))
