...
def unpack_record(raw: bytes, b: int, d: int, k: int, rec_bytes: int
    ...
    # bf16 -> float32: upper 16 bits of a uint32, reinterpret as float
    rnorm_u16 = np.frombuffer(raw[rnorm_off:rnorm_off + 2], dtype=np.uint16)[0]
    r_norm = float(
        np.array([np.uint32(rnorm_u16) << 16], dtype=np.uint32).view(np.float32)[0]
    )
...
def test_binary_record_roundtrip():
    ...
    for v in range(50):
        ...
        # bf16 encode: float32 -> upper 16 bits
        rnorm_f32 = np.array([float(enc['r_norms'][v])], dtype=np.float32)
        rnorm_bf16 = int(rnorm_f32.view(np.uint32)[0] >> 16)
        struct.pack_into('<H', buf, base + pq_bytes + qjl_bytes, rnorm_bf16)
