...
DEFAULT_DIM  = 128
DEFAULT_SEED = 0xDEADBEEF   # kRotationSeed
...
def load_fp16_dump(path: str, n_vectors: int, d: int,
    ...
    arr = all_fp16[:n_vectors].view(np.float16).astype(np.float64)
    return arr
...
def test_device_rotation_matches_reference(
    ...
    x = x_fp16.view(np.float16).astype(np.float64)
    ...
    assert exact_match_rate > 0.95, (
        f"Only {exact_match_rate*100:.1f}% of coordinates match exactly at FP16 "
        f"precision — likely a seed or butterfly ordering mismatch."
    )
...
def test_device_inverse_rotation(
    ...
    n_in = len(raw_in) // (d * 2)
    n = min(n, n_in)
    x = np.frombuffer(raw_in, dtype=np.uint16)[:n * d].reshape(n, d).view(np.float16).astype(np.float64)
    z_dev = z_dev[:n]
