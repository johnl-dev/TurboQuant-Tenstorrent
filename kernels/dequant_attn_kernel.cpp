...
//
// Pipeline:
//   Reads from  tt::CBIndex::c_2  (packed b-bit PolarQuant indices)
//   Reads from  tt::CBIndex::c_16 (QJL bits + BF16 r_norm)
//   Writes to   tt::CBIndex::c_17 (float32 dot product per vector)
//
...
void kernel_main() {
    ...
    for (uint32_t rec = 0u; rec < num_records; ++rec) {
        cb_wait_front(tt::CBIndex::c_2, 1u);
        cb_wait_front(tt::CBIndex::c_16, 1u);
        cb_reserve_back(tt::CBIndex::c_17, 1u);

        volatile uint8_t*  pq_ptr =
            reinterpret_cast<volatile uint8_t*>(get_read_ptr(tt::CBIndex::c_2));
        volatile uint8_t*  qjl_ptr =
            reinterpret_cast<volatile uint8_t*>(get_read_ptr(tt::CBIndex::c_16));
        volatile float*    dot_out =
            reinterpret_cast<volatile float*>(get_write_ptr(tt::CBIndex::c_17));
        ...
        *dot_out = dot_raw + kPiOver2K * r_norm * corr_sum;
        ...
        cb_pop_front(tt::CBIndex::c_2, 1u);
        cb_pop_front(tt::CBIndex::c_16, 1u);
        cb_push_back(tt::CBIndex::c_17, 1u);
    }
}
